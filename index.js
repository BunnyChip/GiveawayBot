...
client.on("ready", async () => {
  console.log(`I am ${client.user.tag} by Neox#1209`)
})
  ...
  if (conditionRole != 'none') {
    if (member.roles.cache.find(r => r.id === conditionRole)) {
      member.send(
        ...
        .setColor('GREEN')
        .setDescription(`Your entry for [this giveaway](https://discordapp.com/channels/${reaction.message.guild.id}/${reaction.message.channel.id}/${giveaway.messageID}) has been approved. **Best of luck!!**`)
...
//add a valid bot token down

client.login('token')

//am doing a lot of discord bot stuffs pls do subscribe me if you like those codes/repls, thank you also dont forget to join my discord - Neox#1209
